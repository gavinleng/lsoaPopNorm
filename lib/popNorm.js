...
module.exports = exports = function(dataArray, normInf, cb) {
	...
	dataDistrict(areaIds, dataArray, normInf, function (dataAll, cids) {
		...
		for (i = 0; i < ageBands.length; i++) {
			// for female
			data.push.apply(data, genderNorm('female', ageBands[i]));
			
			// for male
			data.push.apply(data, genderNorm('male', ageBands[i]));
		}
		...
		var getAllAges = function (dataAll, cb) {
			...
			for (i = 0; i <lenareaIds; i++) {
				// for female
				dataAll.push.apply(dataAll, genderAllAges(dataAll, 'female', year, areaIds[i]));
				
				// for male
				dataAll.push.apply(dataAll, genderAllAges(dataAll, 'male', year, areaIds[i]));
			}
